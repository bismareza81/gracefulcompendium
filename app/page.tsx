...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function fetchArtifacts() {
      try {
        // Fix: Menangani potensi kesalahan "process is not defined" di lingkungan browser
        let supabaseUrl = "";
        let supabaseKey = "";
        ...
        try {
          ...
          console.log("Environment variables not accessible, using mock data.");
        }

        if (!supabaseUrl || !supabaseKey) {
            // Gunakan data simulasi bertema Elden Ring jika Supabase tidak terkonfigurasi
            setBookmarks([
                ...
                { id: 3, title: "Malenia's Winged Helm", category: "Armor", summary: "Helm yang dikenakan oleh Malenia, Blade of Miquella. Simbol kemauan yang tak terjoyahkan.", created_at: new Date().toISOString() },
      ...
      <div className="max-w-7xl mx-auto px-6 py-20 relative z-10">
        ...
        <section className="mb-24 flex flex-col items-center text-center">
            ...
            <h1 className={`${titleFont} text-6xl md:text-[90px] font-black tracking-tight leading-tight uppercase italic mb-8 drop-shadow-lg transition-colors duration-1000 ${isNight ? 'text-[#f5d491]' : 'text-[#4a3a1a]'}`}>
              Graceful <br className="hidden md:block" />
            ...
            <p className="max-w-2xl text-[#8a8a8a] text-lg font-light leading-relaxed mb-12">
              Kumpulan artikel bagi mereka yang mencari kepingan pengetahuan. <br />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            {filteredBookmarks.map((item, i) => (
              <article 
                key={item.id} 
                className={`group relative border p-10 transition-all duration-700 hover:-translate-y-2 flex flex-col ${isNight ? 'bg-[#0d0d0d] border-[#d4ad6a]/5 hover:border-[#d4ad6a]/30 shadow-black' : 'bg-whiteborder-[#a67c00]/5 hover:border-[#a67c00]/30 shadow-xl shadow-stone-200'}`}
              >
                {/* Visual Accent */}
                ...
                <div className="text-left md:text-right">
                    ...
                    <div className="text-[10px] uppercase tracking-[0.3em] text-[#333] font-bold">
                        ...
                        <p className={`mt-2 transition-colors duration-500 hover:text-[#d4ad6a] cursor-default ${isNight ? 'text-[#444]' : 'text-[#999]'}`}>
                            Dibuat dari kebiasaan 積ん読 • Terinspirasi dari Elden Ring
                        </p>
                    </div>
                </div>
            </div>
